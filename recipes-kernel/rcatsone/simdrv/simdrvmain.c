...
static inline int simdrv_Insert  (phoneSIMStruct *p_Dev, struct SimInsert * p_Insert);
static inline int simdrv_ResetSim(phoneSIMStruct *p_Dev, struct SimAtr * p_Atr);
static inline int simdrv_Transact(phoneSIMStruct *p_Dev, struct SimPdu * p_Pdu);

static int fpga_PowerSim  (struct FpgaRegs *p_Fpga, StateChange p_NewState);
static int fpga_ClockSim  (struct FpgaRegs *p_Fpga, StateChange p_NewState);
static int fpga_ResetSim  (struct FpgaRegs *p_Fpga, StateChange p_NewState);
static int fpga_WarmResetSim     (struct FpgaRegs *p_Fpga);
...
//
static int fpga_AttachDevice( struct FpgaRegs *p_Fpga,
    ...
    printk("Out=%d cnt=%d ",cmd.m_Length, tmp);
{
    ...
    printk("%2.2ld.%6.6ld :MonitorOut=%d\n",timeStamp.tv_sec%100, timeStamp.tv_usec, cmd.m_Length);
            ...
            case TEQUAL0_INSERT:
            {
                struct SimInsert            insert;

                if ( copy_from_user(&insert, (uint8_t *)arg, sizeof(struct SimInsert)) )
                    retval = -EFAULT;
                else
                {
            ...
            break;
            case TEQUAL0_TRANSACT:
            {
                struct SimPdu            pdu;

                if ( copy_from_user(&pdu, (uint8_t *)arg, sizeof(struct SimPdu)) )
                    retval = -EFAULT;
                else
                {
                    ...
                    if ( retval > 0 )
                    {
                        // propagate error > 0 Event
                        pdu.m_StatusBlock[0] = 0;
                        pdu.m_StatusBlock[1] = retval;
                        if ( retval == TIMEOUT_EVENT )
                            retval = -ETIMEDOUT;
                        else
                            retval = -ECOMM;
                    }
                    if ( copy_to_user((void*)arg, &pdu, sizeof(struct SimPdu)) )
                        return -EFAULT;
                }
            }
            break;
...
//
// IOCTL function - resets SIM; read ATR and does PPS exchange
//
static inline
int simdrv_ResetSim(phoneSIMStruct *p_Dev, struct SimAtr * p_Atr)
{
    int                 nRetval;
    ...
    // Reset current uart divisor to insure any baud change is applied.
    g_uartDivisor[p_Dev->m_PhoneId] = 0;

    //printk("TEQUAL0 RESET SIM(%d)\n", p_Dev->m_PhoneId);
    // set default baudrate
    //simdrv_SetDefaultBaudrate(p_Dev);
    simdrv_UartConfig(p_Dev, &p_Dev->m_DefaultUartConfig);
    ...
    {
        // < 0 - Linux error; > 0 - T=0 event
        return nRetval;
    }
    // do PPS if we need one
    if ( (p_Atr->m_Atr[1] & (1<<4))  != 0      // if TA1 and requested new baud rate
      && ((p_Atr->m_Atr[2] != 0x11) && (p_Atr->m_Atr[2] != 0x01)))
    {   // do PPS ( protocol and parameter selection
        // 1. send PPS request
        // 2. process PPS response
        nRetval = simdrv_ExecutePps(p_Dev, p_Atr->m_Atr[2]);
    }

    if (nRetval == 0 )
        p_Dev->m_NeedsReset = 0;
    return nRetval;
}
//
//  helper function for simdrv_Transact
//  Function process ACK received from SIM in connection with SIM VPP
//           though our HW does not support it BUT in the feature ...
//
static inline int simdrv_TransactAckVpp(uint8_t p_byCommandCode, uint8_t p_byAck)
{
    uint8_t     byVal = p_byAck ^ p_byCommandCode;
    if ((byVal == 0x00) || (byVal == 0xFF))
    {
        // Turn Off Programming Voltage here (if it were supported)
    }
    if ((byVal == 0x01) || (byVal == 0xFE))
    {
        printk("ERROR: simdrv: Programming Voltage not supported.\n");
        return -EFAULT;
    }
    return 0;
}
//
//  helper function for simdrv_Transact
//  Function processes all possible Procedure bytes
//  Return - 0 - ok; <0 Linux Error or >0 Communication Error
//
//          *p_nNextStep - tells what to do with PDU
//          DONE - PDU processing is finished
//          ALL  - process (send/receive) all remaining payload bytes
//          ONE  - process (send/receive) one remaining payload bytes
//
#define PROCEDURE_DONE  (0)
#define PROCEDURE_ALL   (1)
#define PROCEDURE_ONE   (2)
static inline int simdrv_ReadProcByte( phoneSIMStruct *p_Dev,
                                       struct SimPdu  *p_Pdu,
                                       int *p_nNextStep)
{
    uint8_t     byProcByte;
    int         nRetVal;
    uint8_t     byCommandCode = p_Pdu->m_CommandBlock[1];

    do
    {
        nRetVal = simdrv_ReadData(p_Dev, &byProcByte, 1);
        if ( nRetVal != 0 )
            return nRetVal;         // <0 Linux Error or >0 Communication ErrorCommunication Error (event code)

    } while ( byProcByte == 0x60 );

    // we've got first status byte (SW1)
    if ( (byProcByte & 0xF0) == 0x60 || (byProcByte & 0xF0) == 0x90 )
    {                   // SW1 received; read the SW2 byte and return to caller
        p_Pdu->m_StatusBlock[0] = byProcByte;
        nRetVal = simdrv_ReadData(p_Dev, &p_Pdu->m_StatusBlock[1], 1);
        if ( nRetVal != 0 )
            return nRetVal;         // <0 Linux Error or >0 Communication ErrorCommunication Error (event code)
        *p_nNextStep = PROCEDURE_DONE;
        return 0;
    }

    // we got ACK byte
    // first process ACK's "Result on VPP" part
    if ( simdrv_TransactAckVpp(byCommandCode, byProcByte) != 0 )
        return -EFAULT;                 //????? reset

    if (   (byProcByte & 0xFE) == (byCommandCode & 0xFE) )
        *p_nNextStep = PROCEDURE_ALL;
    else
    if (((~byProcByte) & 0xFE) == (byCommandCode & 0xFE))
        *p_nNextStep = PROCEDURE_ONE;
    else
    {
        p_Pdu->m_StatusBlock[0] = byProcByte;
        return -EPROTO;
...
//      sends 5-bytes command header to the SIM
//      receives ACK/NULL/Status bytes
//      send/receive payload data
//
static inline
int simdrv_Transact(phoneSIMStruct *p_Dev, struct SimPdu * p_Pdu)
{
    int             nRetVal;
    uint32_t        nPayloadLength = p_Pdu->m_CommandBlock[4];
    uint8_t         byCommandCode = p_Pdu->m_CommandBlock[1];
    int             nNextStep;

    // just in case
    if (((byCommandCode & 0xF0) == 0x60) || ((byCommandCode & 0xF0) == 0x90))
        return -EINVAL;

    // return if SIM needs reset
    if (p_Dev->m_NeedsReset)
    {
printk("SIM needs reset\n");
        return -EFAULT;
    }
    if ((nPayloadLength == 0) && p_Pdu->m_PayloadReceive)
        nPayloadLength = 256;

    // send Command Header to the SIM
    nRetVal = simdrv_SendResponse(p_Dev, p_Pdu->m_CommandBlock, sizeof(p_Pdu->m_CommandBlock), 0);
    if ( nRetVal != 0 )
        return nRetVal;         // Linux Error

    nRetVal = simdrv_ReadProcByte(p_Dev, p_Pdu, &nNextStep);
    if ( nRetVal != 0 )
        return nRetVal;         // <0 Linux Error or >0 Communication Error

    p_Pdu->m_PayloadBytes = 0;  // initialize counter
    while ( nNextStep == PROCEDURE_ONE )
    {                   // get ACK - "the next data byte"
                        // SIM can request data byte by byte
        if (p_Pdu->m_PayloadBytes < nPayloadLength)
        {
            if (p_Pdu->m_PayloadReceive)    // read 1 byte from the SIM
                nRetVal = simdrv_ReadData(p_Dev, &p_Pdu->m_Payload[p_Pdu->m_PayloadBytes++] , 1);
            else        // send 1 byte of payload to the SIM
                nRetVal = simdrv_SendResponse(p_Dev, &p_Pdu->m_Payload[p_Pdu->m_PayloadBytes++], 1, 0);

            if ( nRetVal != 0 )
                return nRetVal;             // <0 Linux Error or >0 Communication Error
        }
        else
            return -ENOEXEC;                // should NEVER come here. return distinctive Linux Error
    ...
    if ( nNextStep == PROCEDURE_ALL )
    {                   //get ACK - "all remaining bytes"
                        //---------------------------------------------------------
        if ( p_Pdu->m_PayloadReceive )      // receive data from the SIM
            nRetVal = simdrv_ReadData( p_Dev, &p_Pdu->m_Payload[p_Pdu->m_PayloadBytes],
                                       nPayloadLength - p_Pdu->m_PayloadBytes  );
        else                                // send command payload to the SIM
            nRetVal = simdrv_SendResponse( p_Dev, &p_Pdu->m_Payload[p_Pdu->m_PayloadBytes],
                                            nPayloadLength - p_Pdu->m_PayloadBytes, 0);
        if ( nRetVal != 0 )
            return nRetVal;                 // <0 Linux Error or >0 Communication Error

        p_Pdu->m_PayloadBytes = nPayloadLength;

        nRetVal = simdrv_ReadProcByte(p_Dev, p_Pdu, &nNextStep);
        if ( nRetVal != 0 )
            return nRetVal;                 // <0 Linux Error or >0 Communication Error
    }
    if ( nNextStep != PROCEDURE_DONE)
        return -ENOEXEC;        // should NEVER come here. return distinctive Linux Error
...
// function fills up struct UartRegs with FPGA adresses
//
static inline void fpga_GetFpgaRegisters(int p_PhoneId, struct FpgaRegs *p_Fpga)
{
...
//                   for all phones
//
static void simdrv_TimerFunction(unsigned long p_nJifs)
{
    int     sim;
...
static inline void simdrv_StartInsertTimer(const unsigned long p_nJifs)
{
    ...
    timer_IrCheck.data = (unsigned long)p_nJifs;    // 100 msec
    timer_IrCheck.expires = jiffies + p_nJifs;
    add_timer(&timer_IrCheck);
}
...
//
static inline
int simdrv_Insert(phoneSIMStruct * p_Dev, struct SimInsert * p_Insert)
{
    uint16_t    nNeeded = p_Insert->m_Flags & (BLOCK_IR_EVENT | BLOCK_INSERTED);
    unsigned long    timeout_time = ((p_Insert->m_Timeout != 0) ? (jiffies + p_Insert->m_Timeout) : 0);
    long        nRetWait=0;

    if (p_Insert->m_Timeout)
    {
        nRetWait = wait_event_interruptible_timeout(p_Dev->m_InsertWakeQueue,
                   ((nNeeded = simdrv_WaitInsert(p_Dev, nNeeded)) == 0),
                   p_Insert->m_Timeout );
    }
    else
    {
        nRetWait = wait_event_interruptible(p_Dev->m_InsertWakeQueue,
                   ((nNeeded = simdrv_WaitInsert(p_Dev, nNeeded)) == 0) );
    }

    if ( nRetWait < 0 )         // if signal was received
    {
        if ( p_Insert->m_Timeout )
        {
            unsigned long time_now = jiffies;
            // prevent restart from extending timeout "very far"
            // next two lines uses jiffies function and arithmetic
            if (time_before(time_now, timeout_time))
                p_Insert->m_Timeout = (long)timeout_time - (long)time_now;
            else
                p_Insert->m_Timeout = 1;
        }
        return -ERESTARTSYS;
    }
...
//??????        p_Dev->Flags.MediaChange = 0; // Immaterial at this point, the app knows the truth 8-)
    ...
    // DEBUG ONLY LINES   -
    p_Insert->m_Slot[0] = 'A';
    p_Insert->m_Slot[1] = p_Dev->m_PhoneId + 0x31;      //+'1'
    return 0;
}
...
static int __init simdrv_init_module(void)
{
    ...
    // initialize Insert/Remove timer - each 100 msec
    simdrv_StartInsertTimer((100 * HZ)/1000);
